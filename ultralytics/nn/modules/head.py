...
class Lightweight_Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        self.no = nc + 4  # number of outputs per anchor (nc classes + 4 box coordinates)
        
        # Reduce to a single convolution per layer
        self.cv2 = nn.ModuleList(
            nn.Conv2d(x, 4, 1) for x in ch  # Directly predict 4 bounding box coordinates
        )
        self.cv3 = nn.ModuleList(
            nn.Conv2d(x, self.nc, 1) for x in ch  # Directly predict class probabilities
        )
